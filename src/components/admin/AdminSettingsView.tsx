...
import { DEFAULT_STORAGE_QUOTA_BYTES } from "../../types/index.js";
...
export function AdminSettingsView({ token }: AdminSettingsViewProps) {
  const [settings, setSettings] = useState<any>({
    default_user_quota_bytes: DEFAULT_STORAGE_QUOTA_BYTES, // 15 GB
      ...
      <form onSubmit={handleSave} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs space-y-6">
        ...
        <div>
          ...
          <input
            ...
            min="1"
            max="1024"
            value={Math.round((settings.default_user_quota_bytes || DEFAULT_STORAGE_QUOTA_BYTES) / (1024 * 1024 * 1024))}
            onChange={(e) =>
              setSettings({
                ...settings,
